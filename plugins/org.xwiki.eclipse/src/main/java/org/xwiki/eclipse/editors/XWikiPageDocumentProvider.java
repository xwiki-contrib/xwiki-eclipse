		...
		if (element instanceof XWikiPageEditorInput)
		{
				...
				XWikiEclipseUtil.runOperationWithProgress(
						new IRunnableWithProgress()
						{
									...
									InterruptedException
							{
								...
								String content = input.getXWikiPage()
										.getContent();
			...
			} catch (Exception e)
			{
				throw new CoreException(new Status(IStatus.ERROR,
						XWikiEclipsePlugin.PLUGIN_ID, "Error opening page", e));
			}
		}
			...
			IDocument document, boolean overwrite) throws CoreException
	{
		...
		try
		{
			...
			if (conflictData != null)
			{
				...
				CompareDialog compareDialog = new CompareDialog(Display
						.getDefault().getActiveShell(), conflictData);
